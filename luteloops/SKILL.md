...
- **Protect what the task could edit.** When a milestone's exam lives in files the task itself can touch (tests, fixtures, check scripts), list them under `protected:` (globs, e.g. `["tests/**"]`) on that loop. A run that buys a pass by deleting the failing test instead of fixing the code is then voided to a fail and told to restore it. Opt-in - pair `protected:` with any exam whose materials the task could edit; you decide what counts as the exam.
